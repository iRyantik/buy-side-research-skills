...
`new-session` 是 operations skill。它只负责 topic session scaffold、canonical save path 和 topic `index.md` 的轻量维护，不做研究、不写结论、不推荐下一步 research skill，也不替代 `next-step` 或 `research-journal`。
