...
全局 source / anti-hallucination 规则已内嵌在 `Global Rules Capsule (v1)`。本节只补充 peer-deep-dive-specific 要求。
...
如果运行环境支持 sub-agent，本 skill 可以按公司并行收集 evidence，但只能让 sub-agent 产出 evidence card：
...
N 家公司 × 关键维度的并排数据。**必须有 Source 列**。分两层：

#### 2A. 通用维度（所有行业都列）

| 公司 | 市值 | 收入(LTM) | 收入 YoY | EBITDA margin | ROIC（除现金） | 净负债/EBITDA | Capex/D&A | FCF yield | EV/EBITDA(当前 vs 5Y 中位) | 资本返还/FCF | Source |
|---|---|---|---|---|---|---|---|---|---|---|---|

每行的 Source 列标注主要数据来源（典型：10-K + Q3 10-Q + Bloomberg/CapIQ）。
