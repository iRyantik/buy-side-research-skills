...
| `model_update` | `no` / `actuals_only` / `driver_change` / `assumption_change` | 是否需要触发 `driver-map / model-update` |
...
如果财报暴露披露口径、driver、margin、source 冲突等怪异信号，按 Senior Analyst Radar 直接点破。若 surprise 是机制 / know-how 问题，先触发 `mechanism-insight`；若数字改变的是 revenue / margin / backlog / price-volume-mix 口径，先触发 `driver-map`；若已经进入模型更新，再触发 `driver-map / model-update`。
