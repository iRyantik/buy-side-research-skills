...
全局 source / anti-hallucination 规则已内嵌在 `Global Rules Capsule (v1)`。本节只补充 information-impact-specific 要求。
...
本 skill 默认必须按 claim-check bucket 启动 sub-agent / delegate worker 并行取证；sub-agent 只能返回 evidence card：
