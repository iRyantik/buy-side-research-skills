...
**① 术语纠正**：找出现有 teach-in/quickread 匹配术语，纠正明显错误。**任何实体信息写入最终文件前，至少走一次 WebSearch 核实**——不限于公司名和代码，还包括：上市状态（IPO 中/已上市）、产品名称、行业地位、垄断关系、客户-供应商关系、首字母缩写（CPU/CPO/GPU 等易混淆项）。保留原始文本供 appendix 正名对照使用。不确定标 `[待确认]`。
