#!/usr/bin/env python3
"""Bootstrap ingest dependencies (replaces bootstrap-ingest-deps.ps1 + .sh).

Usage:
  python bootstrap.py              # Install dependencies
  python bootstrap.py --check      # Check status only
  python bootstrap.py --yes        # Skip confirmation
  python bootstrap.py --china      # Use Tsinghua PyPI mirror (China users)
"""

import sys
import os
import subprocess
import importlib.util
import argparse
import json

REQUIRED_PYTHON = (3, 10)
PACKAGES = ["docling", "edgar", "pymupdf4llm", "openpyxl", "python-pptx",
            "python-docx", "pdfplumber", "pypdf", "Pillow"]
SYSTEM_DEPS = {
    "Tesseract OCR": {
        "windows": "choco install tesseract  (or https://github.com/UB-Mannheim/tesseract/wiki)",
        "darwin": "brew install tesseract",
        "linux": "apt-get install tesseract-ocr",
        "check": lambda: os.system("tesseract --version >NUL 2>NUL") == 0 if sys.platform == "win32"
               else os.system("tesseract --version >/dev/null 2>&1") == 0
    }
}

def find_python():
    for name in ["python3", "python"]:
        try:
            result = subprocess.run([name, "-c",
                "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"],
                capture_output=True, text=True)
            if result.returncode == 0:
                ver = tuple(map(int, result.stdout.strip().split(".")))
                if ver >= REQUIRED_PYTHON:
                    return name
        except Exception:
            continue
    return None

def find_requirements():
    script_dir = os.path.dirname(os.path.abspath(__file__))
    candidates = [
        os.path.join(script_dir, "requirements.txt"),
        os.path.join(script_dir, "..", "assets", "requirements-ingest.txt"),
    ]
    for c in candidates:
        if os.path.isfile(c):
            return os.path.normpath(c)
    return None

def check_deps():
    result = {"python": {"available": False, "version": None}, "packages": {}, "system_deps": {}}
    python = find_python()
    if python:
        result["python"]["available"] = True
        v = subprocess.run([python, "-c", "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"],
                          capture_output=True, text=True)
        result["python"]["version"] = v.stdout.strip()
        result["python"]["executable"] = python
    for pkg in PACKAGES:
        spec = None
        try:
            spec = importlib.util.find_spec(pkg.replace("-", "_"))
        except Exception:
            pass
        result["packages"][pkg] = spec is not None
    for name, info in SYSTEM_DEPS.items():
        result["system_deps"][name] = {"installed": info["check"](), "install_hint": info.get(sys.platform, info.get("linux", ""))}
    req_path = find_requirements()
    result["requirements_path"] = req_path
    return result

def main():
    parser = argparse.ArgumentParser(description="Bootstrap ingest dependencies")
    parser.add_argument("--check", action="store_true", help="Check dependency status only")
    parser.add_argument("--yes", action="store_true", help="Skip confirmation prompt")
    parser.add_argument("--china", action="store_true", help="Use Tsinghua PyPI mirror")
    args = parser.parse_args()

    if args.check:
        print(json.dumps(check_deps(), indent=2))
        return

    python = find_python()
    if not python:
        print("ERROR: Python 3.10+ not found. Install from https://python.org", file=sys.stderr)
        sys.exit(1)
    print(f"[OK] Python: {python} ({check_deps()['python']['version']})")

    req_path = find_requirements()
    if not req_path:
        print("ERROR: requirements-ingest.txt not found", file=sys.stderr)
        sys.exit(1)
    print(f"[..] Requirements: {req_path}")

    if not args.yes:
        deps = check_deps()
        missing = [p for p, ok in deps["packages"].items() if not ok]
        if missing:
            print(f"Will install: {', '.join(missing)}")
        else:
            print("All packages already installed. Nothing to do.")
            return
        answer = input("Continue? [y/N] ").strip()
        if answer.lower() not in ("y", "yes"):
            print("Cancelled.")
            sys.exit(1)

    pip_cmd = [python, "-m", "pip", "install"]
    if args.china:
        pip_cmd += ["-i", "https://pypi.tuna.tsinghua.edu.cn/simple"]
    pip_cmd += ["-r", req_path]

    print(f"[..] Running: {' '.join(pip_cmd)}")
    result = subprocess.run(pip_cmd)
    if result.returncode != 0:
        print("ERROR: pip install failed", file=sys.stderr)
        sys.exit(1)

    # Check system deps
    for name, info in SYSTEM_DEPS.items():
        if not info["check"]():
            print(f"[!] {name} not found.")
            print(f"    Install: {info.get(sys.platform, info.get('linux', ''))}")
            print(f"    {name} is required for OCR-based functions only. Non-OCR ingest works without it.")

    print("[OK] ingest dependencies ready.")
    final = check_deps()
    print(json.dumps({k: v for k, v in final.items() if k != "system_deps"}, indent=2))

if __name__ == "__main__":
    main()
