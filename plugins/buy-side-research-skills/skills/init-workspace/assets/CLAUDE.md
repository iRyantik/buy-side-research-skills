...
| `shared/download-image.py` | 图片下载（产品图/logo，workspace 级缓存跨 skill 共享） | `--logo <TICKER>` 或 `<url> --output <slug>` |
...
- Logo：`download-image.py --logo <TICKER>`（自动缓存，同一 ticker 全 workspace 只下一次）
