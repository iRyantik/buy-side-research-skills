...
- `topic-local evidence cache` means this research workspace's topic `_cache/`, company `financial-data`, Source Intake-tracked markdown, and saved internal data packs; it is different from home-market / local-language source priority.
...
- `topic-local evidence cache`：当前 topic `_cache/`、company `financial-data`、Source Intake 生成的 source-tracked markdown、已保存的内部数据包；不包含 research artifacts，它们只能作为导航、routing 和待复核线索。
...
- Workspace routing: 研究 skill 保存 artifact 时自动创建缺失目录（`mkdir -p`）；Source Intake Router 在首次发布时创建 `_raw/<category>/` 和 `_cache/<category>/`。Industry/theme topics may hold single-company workbench files named `YYYY-MM-DD-<company-slug>-<artifact>.md`。`integrate` remains whole-topic directory merge。
...
- Source Intake publish 前确保 topic root 已存在（`industry/<industry>/index.md` 必须存在）。若缺失，agent 自动创建。
- 研究 skill 启动时，先检查 `industry/<industry-slug>/_cache/` 是否存在 Source Intake 已发布的相关材料。如有，优先引用 cache 中的 source-tracked markdown，而非重新获取原始文件。若是单公司研究，同时检查相关 `industry/<industry>/companies/<ticker>/_cache/financial-data/internal/actuals-resolved.json`；需要审计或机器输入时再进入 `internal/facts-store.json` 和 run-id evidence pack。
