...
- `topic-local evidence cache`: current topic `_cache/`, company `financial-data`, Source Intake-tracked Markdown, and saved internal data packs; it does not include research artifacts, which can only serve as navigation, routing, and pending-review clues.
...
`financial-data` writes canonical facts to `_cache/financial-data/internal/facts-store.json` and generates `_cache/financial-data/internal/actuals-resolved.json` as a read-only compatibility view. Research skills consume the generated view by default and must not bypass canonical financial-data to fetch raw financials directly.
...
| Register, convert, and route raw materials | Source Intake runtime |
