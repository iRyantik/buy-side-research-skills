...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--market", choices=sorted(MARKET_MODULES), default="us")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.all_companies:
        companies = [path for path in (workspace / "industry").glob("*/companies/*") if path.is_dir()]
    else:
        if not args.ticker and not args.company_root:
            raise SystemExit("ticker or --company-root is required")
        companies = [
            Path(args.company_root).resolve()
            if args.company_root
            else _find_company(workspace, args.ticker)
        ]
